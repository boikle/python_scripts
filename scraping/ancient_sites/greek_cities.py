...
def main():
    ...
    rows = []
    fields = ['city_name', 'lat', 'long', 'founded', 'abandoned', 'source_url']
    ...
    for link in links:
        ...
        city_name = link.text
        abandoned = utils.get_abandonded_date(city_url)
        founding = utils.get_founding_date(city_url)
        city_coords = utils.get_city_coords(city_url)
        if city_coords:
            print("{}, {}, {}, {}, {}".format(city_name, utils.parse_lat(city_coords), utils.parse_long(city_coords), founding, abandoned))
            rows.append([city_name, utils.parse_lat(city_coords), utils.parse_long(city_coords), founding, abandoned, city_url])
